...
export default function AdminPlans() {
  ...
  const [form, setForm] = useState({ id: 0, name: '', description: '', price: 0, duration: 30, max_classes: 0, active: true, trial_price: 0, trial_days: 0 })
  ...
  const save = async () => {
    try {
      const data = { name: form.name, description: form.description, price: form.price, duration: form.duration, max_classes: form.max_classes, trial_price: form.trial_price || 0, trial_days: form.trial_days || 0 } as any
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-4">
        ...
        <Button size="sm" onClick={() => { setForm({ id: 0, name: '', description: '', price: 0, duration: 30, max_classes: 0, active: true, trial_price: 0, trial_days: 0 }); setShowForm(true) }}>+ Nuevo</Button>
      </div>
      {loading ? <p className="text-muted text-center py-8">Cargando...</p> : (
        <div className="space-y-2">
          {items.map(p => (
            <Card key={p.id} className="flex items-center justify-between cursor-pointer" onClick={() => { setForm({ id: p.id, name: p.name, description: p.description, price: p.price, duration: p.duration, max_classes: p.max_classes, active: p.active, trial_price: p.trial_price || 0, trial_days: p.trial_days || 0 }); setShowForm(true) }}>
              <div>
                <p className="font-medium">{p.name}</p>
                <p className="text-sm text-muted">{fmtCLP(p.price)} &middot; {p.duration} días &middot; {p.max_classes === 0 ? 'Ilimitado' : `${p.max_classes} clases`}</p>
                {p.trial_price && p.trial_days ? (
                  <p className="text-xs text-warning">Trial: {fmtCLP(p.trial_price)} primeros {p.trial_days} días</p>
                ) : null}
              </div>
      ...
      <Modal open={showForm} onClose={() => setShowForm(false)} title={form.id ? 'Editar plan' : 'Nuevo plan'}>
        <div className="space-y-3">
          ...
          <p className="text-xs text-muted font-medium pt-1">OFERTA TRIAL (opcional)</p>
          <div className="flex gap-2">
            <Input type="number" placeholder="Precio trial (CLP)" value={form.trial_price || ''} onChange={e => setForm(f => ({ ...f, trial_price: Number(e.target.value) }))} />
            <Input type="number" placeholder="Días elegibles" value={form.trial_days || ''} onChange={e => setForm(f => ({ ...f, trial_days: Number(e.target.value) }))} />
          </div>

...
export default function AdminRoutines() {
  ...
  const [form, setForm] = useState({ id: 0, name: '', description: '', type: 'wod', content: '', duration: 0, difficulty: '' })
  ...
  const save = async () => {
    try {
      if (form.id) await routApi.update(form.id, { name: form.name, description: form.description, type: form.type, content: form.content, duration: form.duration || undefined, difficulty: form.difficulty } as any)
      else await routApi.create({ name: form.name, description: form.description, type: form.type, content: form.content, duration: form.duration || undefined, difficulty: form.difficulty } as any)
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-4">
        ...
        <Button size="sm" onClick={() => { setForm({ id: 0, name: '', description: '', type: 'wod', content: '', duration: 0, difficulty: '' }); setShowForm(true) }}>+ Nueva</Button>
      </div>
      ...
      {loading ? <p className="text-muted text-center py-8">Cargando...</p> : (
        <div className="space-y-2">
          {items.map(r => (
            <Card key={r.id} className="cursor-pointer" onClick={() => { setForm({ id: r.id, name: r.name, description: r.description, type: r.type, content: r.content, duration: r.duration, difficulty: r.difficulty }); setShowForm(true) }}>
      ...
      <Modal open={showForm} onClose={() => setShowForm(false)} title={form.id ? 'Editar rutina' : 'Nueva rutina'}>
        <div className="space-y-3">
          ...
          <Textarea placeholder="Contenido del WOD" value={form.content} onChange={e => setForm(f => ({...f, content: e.target.value}))} rows={6} />
